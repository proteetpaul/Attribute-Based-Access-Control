...
def range_search(node, dist, reslist, point, cdimen, ddimen, nodesVisited):
    nodesVisited += 1
    n = len(node.dmbrs)
    if node.isLeaf == True:
        for i in range(0,n):
            ...
            if dist >= calcDist(point, rec, cdimen, ddimen):
                reslist.append(rec)
        return nodesVisited
    for i in range(0,n):
        rec = hds_rectangle()
        ...
        rec.d_arr = node.dmbrs[i]
        if dist >= calcDist(point, rec, cdimen, ddimen):
            nodesVisited = range_search(node.children_[i], dist, reslist, point, cdimen, ddimen, nodesVisited)
    return nodesVisited
...
def func(nr):
    ...
    for i in range(0,len(queries)):
        ...
        for dist in query:
            reslist = []
            start = time.time()
            nodesVisited = range_search(root, dist, reslist, points[i], cdimen, ddimen, 0)
            end = time.time()
    ...
    resultsfile = open(str(nr)+"range_results.txt","w")
